...
void * tarefa(void *arg) {
   ...
   for(int i = args->id; i < args->dim; i += args->nthreads){ //linhas alternadas, saltando o número de threads
      for(int j = 0; j < args->dim; j++) 
         saida[i] += mat[i*(args->dim) + j] * vet[j]; //tem como digitar isso não, mas mat[i *(args->dim) + j] == mat[i][j]
   }   
   pthread_exit(NULL);
}
